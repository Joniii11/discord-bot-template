import { APIApplicationCommandOption, APIApplicationCommandSubcommandOption, ApplicationCommandDataResolvable, ApplicationCommandOption, ApplicationCommandOptionType, Collection, RESTPostAPIChatInputApplicationCommandsJSONBody } from "discord.js";
import DiscordBot from "../structures/DiscordBot.js";

export async function registerSlashCommands(client: DiscordBot, force = false) {
    if (!force) {
        if (!await checkCommandsUpdate(client)) return client.logger.log("[Command Manager] Slash commands are the same as before, will not update!", "debug");
    };
     ...
     try {
        ...
        const cmdData:RESTPostAPIChatInputApplicationCommandsJSONBody[] = []
        client.manager.commandManager.getCommands.forEach((cmd) => {
            cmdData.push(cmd.data.toJSON() as RESTPostAPIChatInputApplicationCommandsJSONBody)
        });
 ...
 */
export async function checkCommandsUpdate(client: DiscordBot): Promise<boolean> {
    ...
    if (commands) {
        ...
        for (const command of commands) {
            ...
            if (newCommand.data.name !== command[1].name) return true;
            // Check if the description is not equal or not
            if (newCommand.data.description !== command[1].description) return true;

            if (!newCommand.data.options) continue;
            ...
            for (let index = 0; index < newCommand.data.options.length; index++) {
                ...
                if (!currentLocalCommand || !currentDiscordCommand) return true;

                if (currentCommands(currentLocalCommand, currentDiscordCommand)) return true;
            }
        }
 ...
 */
function currentCommands(currentLocalCommand: APIApplicationCommandOption, currentDiscordCommand: ICurrentDiscordCommand): boolean {
    // Check if the name of the option needs to be updated
    if (currentLocalCommand.name !== currentDiscordCommand?.name) return true;

    // Check if the description of the option needs to be updated
    if (currentLocalCommand.description !== currentDiscordCommand.description) return true;

    // Check if the localized description needs to be updated
    if (currentLocalCommand.description_localizations !== currentDiscordCommand.descriptionLocalized) return true;

    // Check if the localized name needs to be updated
    if (currentLocalCommand.name_localizations !== currentDiscordCommand.nameLocalized) return true;
    ...
    switch (currentLocalCommand.type) {
        case ApplicationCommandOptionType.String:
            ...
            if (currentLocalCommand.type !== currentDiscordCommand.type) return true;

            // Check if the max length of the option needs to be updated
            if (currentLocalCommand.max_length !== currentDiscordCommand.maxLength) return true;

            // Check if the min length of the option needs to be updated
            if (currentLocalCommand.min_length !== currentDiscordCommand.minLength) return true;

            // Check if the required property needs to be updated
            if (!compareBools((currentLocalCommand.required ?? false), (currentDiscordCommand.required ?? false))) return true;
            break;
        ...
        case ApplicationCommandOptionType.Subcommand:
            ...
            if (!currentLocalCommand.options || !currentDiscordCommand.options) break;

            for (let i = 0; i < currentLocalCommand.options.length; i++) {
                // Check if the name of the option needs to be updated
                if (currentLocalCommand.options[i]?.name !== currentDiscordCommand.options[i]?.name) return true;

                // Check if the description of the option needs to be updated
                if (currentLocalCommand.options[i]?.description !== currentDiscordCommand.options[i]?.description) return true;

                // Check if the type of the option needs to be updated
                if (currentLocalCommand.options[i]?.type !== currentDiscordCommand.options[i]?.type) return true;

                // Check if the required property needs to be updated
                if (!compareBools((currentLocalCommand.options[i]?.required ?? false), (currentDiscordCommand.options[i]?.required ?? false))) return true;
            }
            break;
        ...
        case ApplicationCommandOptionType.SubcommandGroup:
            ...
            for (const opt of currentLocalCommand.options) {
                // Check if the name of the option needs to be updated
                if (opt.name !== currentDiscordCommand.options[counter]?.name) return true;

                // Check if the description of the option needs to be updated
                if (opt.description !== currentDiscordCommand.options[counter]?.description) return true;
        ...
        case ApplicationCommandOptionType.Integer:
            ...
            if (currentLocalCommand.max_value !== currentDiscordCommand.maxValue) return true;

            // Check if the min value of the option needs to be updated
            if (currentLocalCommand.min_value !== currentDiscordCommand.minValue) return true;

            // Check if the autocomplete property needs to be updated
            if (!compareBools((currentLocalCommand.autocomplete ?? false), (currentDiscordCommand.autocomplete ?? false))) return true;
        ...
        case ApplicationCommandOptionType.Channel:
            ...
            if (currentLocalCommand.channel_types !== currentDiscordCommand.channelTypes) return true;
...
/**
 * Interface representing a Discord command with additional localized properties.
 */
type ICurrentDiscordCommand = ApplicationCommandOption & {
